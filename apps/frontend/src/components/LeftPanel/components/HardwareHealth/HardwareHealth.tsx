...
export function HardwareHealth({
  ...
  const getBatteryColor = (val: number) => {
    if (val < 15) return "text-red-500 border-red-500/20 bg-red-500/10 shadow-[inset_0_0_15px_rgba(239,68,68,0.1)]";
    if (val < 30) return "text-yellow-500 border-yellow-500/20 bg-yellow-500/10 shadow-[inset_0_0_15px_rgba(234,179,8,0.1)]";
  ...
  const getTempColor = (val: number) => {
    ...
    if (val > 60) return "text-yellow-500 border-yellow-500/20 bg-yellow-500/10";
  ...
  const getLatencyColor = (val: number) => {
    ...
    if (val > 150) return "text-yellow-500 border-yellow-500/20 bg-yellow-500/10";
  ...
  const getRssiColor = (val: number) => {
    ...
    if (val < -75) return "text-yellow-500 border-yellow-500/20 bg-yellow-500/10";
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="grid grid-cols-2 gap-3 pb-4">
        {/* Battery Container */}
        <div className={cn("flex flex-col items-center justify-center gap-2 rounded-lg border p-3 transition-colors", getBatteryColor(battery))}>
        ...
        <div className={cn("flex flex-col items-center justify-center gap-2 rounded-lg border p-3 transition-colors", getTempColor(temp))}>
        ...
        {/* RSSI Container */}
        <div className={cn("flex flex-col items-center justify-center gap-2 rounded-lg border p-3 transition-colors", getRssiColor(rssi))}>
        ...
        {/* Latency Container */}
        <div className={cn("flex flex-col items-center justify-center gap-2 rounded-lg border p-3 transition-colors", getLatencyColor(latency))}>
